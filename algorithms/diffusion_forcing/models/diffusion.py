...
class Diffusion(nn.Module):
    ...
    def ddpm_sample_step(
        ...
        noise = torch.where(
            (clipped_curr_noise_level > 0)[..., None, None, None],
            torch.randn_like(x),
            0,
        )
        noise = torch.clamp(noise, -self.clip_noise, self.clip_noise)
        ...
        return torch.where((curr_noise_level == -1)[..., None, None, None], x, x_pred)
    ...
    def ddim_sample_step(
        ...
        return torch.where(
            (curr_noise_level == next_noise_level)[..., None, None, None],
            x,
            x_pred,
        )
